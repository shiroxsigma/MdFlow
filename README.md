...
- 常時表示の経路ボタンによるプリセット選択と、自動判定結果の強調表示
...
エディタ下部の「表示経路」からプリセットをワンクリックで切り替えられます。「条件から自動」を
選んだ場合は、条件JSONに一致した経路も枠線で示します。JSON入力は必要なときだけ展開できます。
...
エディタ下部の「対象図」でMermaid図を選び、「全経路を作成」を押すと、選択中の
flowchart／stateDiagramを解析し、開始から終了までの経路を
`自動経路 01`形式のプリセットとして`mdflow-mapping`へ保存します。手動プリセットは維持され、循環は
同じ遷移を二度通らない有限経路として扱います。日本語の状態名と`[*]`開始／終了にも対応します。
確認ダイアログなしで作成され、`Ctrl+Z`で元に戻せます。
経路爆発を防ぐため一度に生成するのは最大100件です。
